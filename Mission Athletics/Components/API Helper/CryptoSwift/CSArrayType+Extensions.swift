...
public extension CSArrayType where Iterator.Element == UInt8 {
    ...
    func sha1() -> [Iterator.Element] {
        return Digest.sha1(cs_arrayValue())
    }

    func sha224() -> [Iterator.Element] {
    ...
    func sha384() -> [Iterator.Element] {
        return Digest.sha384(cs_arrayValue())
    }

    func sha512() -> [Iterator.Element] {
        return Digest.sha512(cs_arrayValue())
    }

    func sha2(_ variant: SHA2.Variant) -> [Iterator.Element] {
        return Digest.sha2(cs_arrayValue(), variant: variant)
    }

    func sha3(_ variant: SHA3.Variant) -> [Iterator.Element] {
    ...
    func encrypt(cipher: Cipher) throws -> [Iterator.Element] {
        return try cipher.encrypt(cs_arrayValue().slice)
    }

    func decrypt(cipher: Cipher) throws -> [Iterator.Element] {
        return try cipher.decrypt(cs_arrayValue().slice)
    }

    func authenticate<A: Authenticator>(with authenticator: A) throws -> [Iterator.Element] {
